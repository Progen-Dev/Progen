...
public class CommandPrivateVoiceChannel extends CommandHandler {

    public CommandPrivateVoiceChannel() {
        super("pc",
    ...
    private void addUserToChannel(MessageReceivedEvent event) {
        ...
        if (!checkOwnership(channel, member)) {
            event.getTextChannel()
                    .sendMessageEmbeds(super.messageGenerators.generateErrorMsg("This is not your voice channel!")).queue();
            return;
        }
    ...
    private void createChannel(MessageReceivedEvent event, String categoryID) {
        ...
        event.getGuild().createVoiceChannel(PRIVATE_CHANNEL_PREFIX + " " + member.getUser().getName())
                ...
                .queue(channel -> {
                    ...
                    event.getTextChannel().sendMessageEmbeds(super.messageGenerators.generateSuccessfulMsg()).queue();
                });
    }
